...
import { deploymentOnlyPackageName } from './release/deployment-policy.ts'
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  // Product providers replace the default trigger plugin through this standalone browser entry.
  '@deepseek-ai/dsh-client-ui-input-trigger': ['lib/controller.js'],
...
function isReleaseMemberDirectory(dir: string): boolean {
  return deploymentOnlyPackageName(dir) === undefined
 ...
 * @param directory - normalized package directory when checking a deployment-only root.
 ...
 */
export function checkDshFamilyVersion(manifest: PackageManifest, expected: string | undefined, directory?: string): string | undefined {
  const name = manifest.name
  if (directory !== undefined && name === deploymentOnlyPackageName(directory)) return undefined
...
export function checkWorkspaceManifest({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  const label = manifest.name ?? dir
  const deploymentName = deploymentOnlyPackageName(dir)
  const familyVersionError = checkDshFamilyVersion(manifest, repositoryVersion, dir)
  if (familyVersionError !== undefined) errors.push(familyVersionError)
  if (deploymentName !== undefined) {
    if (manifest.name !== deploymentName) errors.push(`${dir}: deployment-only root must be named ${deploymentName}`)
    if (manifest.private !== true) errors.push(`${label}: deployment-only root must set "private": true`)
    if (manifest.publishConfig !== undefined) errors.push(`${label}: deployment-only root must omit publishConfig`)
  }
  ...
  if (dir.startsWith('apps/') && dir !== desktopApplicationDirectory && deploymentName === undefined
