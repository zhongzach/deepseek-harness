...
describe('release families', () => {
  it('excludes the deployment-only peer closure from publication and shared-version rewrites', () => {
    const root = mkdtempSync(join(tmpdir(), 'dsh-release-closure-'))
    roots.push(root)
    write(join(root, 'package.json'), '{"version":"0.1.5-alpha.2"}\n')
    write(join(root, 'apps/public/package.json'), '{"name":"@deepseek-ai/dsh-public","version":"0.1.5-alpha.2"}\n')
    const closure = { name: '@deepseek-ai/dsh-closure', version: '0.0.1', private: true }
    write(join(root, 'apps/closure/package.json'), JSON.stringify(closure))
    const dsh = releaseFamily('dsh')
    const members = dsh.members(root)
    expect(members.map(entry => entry.name)).toEqual(['@deepseek-ai/dsh-public'])
    expect(planShared(dsh, root, members, '0.1.5-alpha.3').planned.map(entry => entry.manifestPath))
      .toEqual(['package.json', 'apps/public/package.json'])
    for (const invalid of [
      { ...closure, private: false },
      { ...closure, name: '@deepseek-ai/dsh-other' },
      { ...closure, publishConfig: { access: 'public' } },
    ]) {
      write(join(root, 'apps/closure/package.json'), JSON.stringify(invalid))
      expect(() => dsh.members(root)).toThrow(/private deployment-only package/)
    }
  })
