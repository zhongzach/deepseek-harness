...
产品可以通过可释放的 locale 覆盖、导航策略、目录流准备回调和可选改名对话框插槽适配 Workspace 展示。这些 UI 扩展保留原生 Workspace 和 Session 对象；具体行为由 [Workspace UI 包](../../packages/client/ui-workspace/README.zh.md)定义。
