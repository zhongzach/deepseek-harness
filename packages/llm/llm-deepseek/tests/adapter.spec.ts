...
import LlmRuntime, { createAssistantMessage, createSystemMessage, createToolResultMessage, createUserMessage, LlmError, ToolCallId } from '@deepseek-ai/dsh-llm'
...
describe('direct Messages HTTP', () => {
  ...
  it('lists no models while the route cannot authenticate', async () => {
    const unavailable = new DeepSeekAdapter({
    ...
    await expect(unavailable.listModels('deepseek-official')).resolves.toEqual([])
  })
  ...
  it('propagates credential failures other than a missing credential from model listing', async () => {
    ...
    await expect(failing.listModels('deepseek-official')).rejects.toMatchObject({ code: 'INVALID_CREDENTIAL' })
  })
