...
Model discovery returns an empty catalog while the referenced credential is missing. The route stays registered, so adding a credential exposes its configured models on the next catalog read. Other credential failures remain visible as provider-local discovery errors.
