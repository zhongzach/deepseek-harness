...
提供方和模型元数据用于发现，不构成路由白名单。`registerAdapter()` 仍拥有提供方路由的排他性，并为每条路由捕获适配器的重试策略；适配器可以接受未出现在 `listModels()` 中的模型 id，消费方不得仅因模型未列出而拒绝请求。返回的 selector 元数据已分离；可选的 `presentation` 元数据可以用稳定的 `sectionId`／`sectionName` 和可选顺序把模型放进展示分区，但不会改变其提供方／模型身份，也不会进入任何请求。无效或重复的适配器条目会以 `INVALID_ADAPTER` 或 `INVALID_CATALOG` 失败。
