...
部署插件可以调用 `applyWithExtension`，而不再单独挂载通用插件。纯函数 `configure` 投影为校验和模型解析提供运行时默认值，不会改写 `llm-pi-ai` 设置分节。`prepareRequest` 在读取凭据前接收已捕获的 profile、模型和已校验的推理等级；它可以拒绝请求，或返回仅用于本次请求的载荷转换函数。转换函数必须保留消息、工具、模型标识和所选协议。鉴权、流式处理、注册释放和重试仍由本适配器负责；Chat Completions 的思考内容回传在转换之后运行。不提供扩展时，通用插件保持原有行为。

模型条目和覆盖项接受字符串值的 `metadata`，用于部署策略，不得存放秘密。适配器随每份 profile 捕获独立的注解映射；默认情况下，注解不进入 pi-ai 模型或请求。`describeReasoning` 可以按实际分派含义设置已支持档位 id 的名称与顺序，但不能改变 id 或增加不支持的选项。描述与请求钩子读取同一份已捕获注解，因此显示名称与 id 不同时，已保存选择仍保留原含义。
