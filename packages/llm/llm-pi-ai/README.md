...
How a thinking level travels — `reasoning_effort` alone, DeepSeek's `thinking: {type}` plus effort, z.ai's `thinking` object, and so on — is pi-ai's `compat.thinkingFormat`, which pi-ai guesses from the endpoint URL; a private gateway's URL says nothing, so a DeepSeek-dialect gateway would be spoken to in the OpenAI dialect with no way to correct it. `compat.thinkingFormat`, `compat.supportsReasoningEffort`, `compat.supportsUsageInStreaming`, `compat.maxTokensField`, `compat.supportsStore`, and `compat.supportsDeveloperRole` are therefore configurable on the route (its models' default) and per model (winning per field), resolving model → route → installed catalog entry → pi-ai's URL-derived guess; setting a route-level switch shadows the catalog entry's value for every model on the route, and there is no spelling for handing a field back to the catalog short of restating its value. `supportsUsageInStreaming: false` makes pi-ai omit `stream_options: { include_usage: true }` — the escape hatch for a gateway whose field whitelist rejects the standard field. `maxTokensField: 'max_tokens'` names the output cap the way most OpenAI-compatible vendors and gateways expect (pi-ai's guess for a URL it does not recognize is the OpenAI-native `max_completion_tokens`, which such an endpoint may reject). `supportsStore: false` keeps OpenAI's `store` field off the wire (pi-ai sends `store: false` to every URL it does not recognize as non-standard; a vendor that rejects unknown parameters answers 400). `supportsDeveloperRole: false` keeps a reasoning model's system prompt on the universal `system` role (pi-ai puts it on OpenAI's `developer` role for every URL it does not recognize as non-standard; a vendor that only knows `system` rejects the request). `thinkingFormat` accepts pi-ai's dispatchable formats except the two `chat-template` variants, which need `chatTemplateKwargs` this configuration does not expose. All six switches exist only on `openai-completions` — the other protocols carry their reasoning shape in the protocol itself — so a model-level switch elsewhere fails resolution, a route-level one skips models of other protocols, and a route with no `openai-completions` model at all is refused. The rest of pi-ai's compat surface (`requiresToolResultName`, …) stays auto-detected and is deliberately not configurable here.

One repair on the Chat Completions wire needs no switch. A DeepSeek-style thinking endpoint (the official API, and vLLM / SGLang deployments running the DeepSeek chat template) streams reasoning as `reasoning_content` and demands the field back on every assistant message of the current tool-call round — `400 messages[N].reasoning_content is required for thinking tool-call history` otherwise, and the round dies mid-work. pi-ai replays the field for every step that produced reasoning, but a step the endpoint answered without any (an empty thinking block some deployments emit for a trivial tool call) leaves a hole pi-ai cannot fill: it only writes non-empty thinking. The adapter therefore fills such holes with `""` through pi-ai's `onPayload` hook — presence is what the check wants — and only in a request that already carries `reasoning_content` on some assistant message, so an endpoint that never spoke the field never sees it (OpenAI proper rejects unknown message keys).
