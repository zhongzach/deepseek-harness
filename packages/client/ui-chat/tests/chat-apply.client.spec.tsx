...
import type { TurnTailMetricsInjected } from '../src/client/chat/TurnTailNodeView.tsx'
...
describe('Chat apply wiring', () => {
  it.each([true, false])('passes the composition metric visibility %s into the turn renderer', async (showTurnMetrics) => {
    const b = await bench()
    const row = b.runtime.slots.entries('conversation.chat.node').find(entry => entry.options.key === 'turn-tail')
    const face = (row?.inject as unknown as () => TurnTailMetricsInjected)()
    expect(face.hooks.showTurnMetrics.getSnapshot()).toBe(false)
    b.chatSettings.publish({
      status: 'ready', value: { transcriptView: 'compact', showTurnMetrics }, revision: 1, writable: true,
    })
    expect(face.hooks.showTurnMetrics.getSnapshot()).toBe(showTurnMetrics)
