...
function makeHarness(
  ...
  const renderNodeSlot = ((key: string, owner: object, opts?: {
    ...
    switch (nodeOwner.node.kind) {
      ...
      case 'context':
        return <ContextMessageNodeView {...nodeProps<'context'>()} SessionProvider={props.SessionProvider} renderSlot={{}} renderSlotChain={(_key, _owner, options) => options?.fallback ?? null} />
      case 'assistant-step':
...
describe('ChatView', () => {
  ...
  it('folds Think and Tool rows before the final answer without unmounting them', () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    const toggle = view.getByRole('button', { name: '1 次工具调用 · 1 条消息 · 1 个协作助手' })
    ...
    const renewedToggle = view.getByRole('button', { name: '1 次工具调用 · 1 条消息 · 1 个协作助手' })
  ...
  it('keeps a focused process row visible when a live Turn completes', () => {
    ...
    const view = render(<h.ChatView {...h.props} />)
    const contextToggle = view.getByRole('button', { name: '已载入写作环境' })
    const contextRow = view.container.querySelector<HTMLElement>('[data-chat-flow-kind="context"]')
    contextToggle.focus()
  ...
  it('can omit generated metric controls without hiding the answer or message actions', () => {
    const h = makeHarness({
      nodes: [user(1, 'hi'), assistant(2, 'final answer')],
      turnTimings: new Map([[1, { startTime: 1_000, endTime: 20_000 }]]),
      turnEnds: new Map([[1, 20]]),
      turnUsages: new Map([[1, {
        uncachedInputTokens: 5060, cacheReadTokens: 4940, outputTokens: 100, totalTokens: 10100,
      }]]),
    }, {}, undefined, false)
    const view = render(<h.ChatView {...h.props} />)
    expect(view.getByText('final answer')).toBeTruthy()
    ...
    expect(view.queryByRole('button', { name: /用时|用量/ })).toBeNull()
    expect(view.container.querySelector('[data-turn-tail="1"]')?.textContent).not.toMatch(/tok|token|19秒/i)
  })
