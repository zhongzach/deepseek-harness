...
组合配置 `showTurnMetrics` 默认为 `true`。Host 将其发布到 `ui-chat` 设置基础层，Client 通过现有设置 scope 读取同一命名空间。设置加载期间控件保持隐藏。解析结果为 `false` 时，消息操作旁不显示每轮 token 与耗时控件；回答、复制与分支操作、持久化用量以及独立注册的统计仍保留。该偏好不是模型请求参数。

产品可通过 `conversation.chat.context.presentation` 链按来源定制上下文呈现，`ContextPresentationOwnerProps` 提供准确的 Chat 上下文数据；链未接管时保留标准折叠行。已完成的 Chat 行使用浏览器内容可见性机制，减少产品面板缩放时的布局开销。
