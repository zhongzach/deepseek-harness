...
The composition option `showTurnMetrics` defaults to `true`. The Host publishes it into the `ui-chat` settings base; the Client reads that namespace through the existing settings scope. Controls remain hidden while settings load. A resolved `false` omits the per-Turn token and timing controls beside message actions; answers, copy/fork actions, durable usage, and independently registered statistics remain available. This preference is not a model-request parameter.

Products may intercept specific context producers through the `conversation.chat.context.presentation` chain; `ContextPresentationOwnerProps` supplies the exact Chat context data. Declining the chain retains the standard disclosure row. Settled Chat seats use browser content visibility to reduce layout work when product panels resize.
