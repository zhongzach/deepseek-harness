...
const ChatNodeList = memo(function ChatNodeList({ entries, useChatGroup, pendingInputs, lastInputTurn, ...seatProps }: ChatNodeListProps) {
  ...
  const pendingRows = pendingInputs.map(item => 'requestId' in item ? (
    <PendingSubmissionBubble key={item.requestId} submission={item}
      ...
      renderUserText={seatProps.renderUserText} openFile={seatProps.openFile} t={seatProps.t} />
  ) : (
...
export function ChatView({
  ...
  renderSlot, renderSlotChain, sessionId, openFile, openSkill, openExternalLink, loadOlder, loadThrough, loadImage,
  inspectCall, chatScroll, forkAt, fileMentions, usePresentation, useProjection, t,
}: ChatViewSlotProps) {
