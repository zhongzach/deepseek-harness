...
describe('UiWorkspaceService', () => {
  it('applies and disposes a product navigation policy without changing native defaults', async () => {
    ...
    const clear = vi.spyOn(b.layout, 'selectPanel')
    b.uiWorkspace.startSession(); expect(empty).toHaveBeenCalledOnce(); expect(clear).not.toHaveBeenCalled()
    undo(); expect(b.uiWorkspace.transferDraftOnSwitch).toBe(true)
    b.uiWorkspace.startSession(); expect(clear).toHaveBeenCalledWith(null)
