...
describe('workspace browser rows', () => {
  ...
  it('shows descendant activity without describing an idle parent as running', () => {
    ...
    try {
      ...
      expect(screen.getByText('2 个协作助手运行中')).toBeTruthy()
      expect(screen.queryByText('进行中')).toBeNull()

      fireEvent.pointerEnter(row.parentElement as HTMLElement)
      act(() => { vi.advanceTimersByTime(500) })
      expect(screen.getAllByText('2 个协作助手运行中')).toHaveLength(2)
    } finally {
      vi.useRealTimers()
    }
  })
  ...
  it('keeps descendant activity secondary while the parent is running', () => {
    ...
    try {
      ...
      expect(screen.getByText('进行中')).toBeTruthy()
      expect(screen.getByText('1 个协作助手运行中')).toBeTruthy()

      fireEvent.pointerEnter(row.parentElement as HTMLElement)
      act(() => { vi.advanceTimersByTime(500) })
      expect(screen.getAllByText('进行中')).toHaveLength(2)
      expect(screen.getAllByText('1 个协作助手运行中')).toHaveLength(2)
    } finally {
      vi.useRealTimers()
    }
  })
