...
// satisfies the generic render signature only with an erased owner type. A
// provided fallback wins so slot-less holes (the rename dialog) render their
// native counterpart.
const renderDirectoryFlowOnly: WorkspaceBrowserProps['renderSlot'] = (name: string, owner: object, options?: { fallback?: ReactNode }) =>
  options?.fallback ?? (name === 'sidebar.workspaces.directoryFlow' && (owner as DirectoryFlowOwnerProps).open
...
describe('Workspace tree grouping', () => {
  ...
  it('adopts a parent directory and opens its Session without confirmation', async () => {
    ...
    await waitFor(() => { expect(b.props.startSession).toHaveBeenCalledWith(wid('root'), undefined) })
