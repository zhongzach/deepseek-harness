...
`./client/controller` 是 ESM 构建期库入口，导出同一份 `InputTriggerController` 及其依赖和来源表类型，不包含插件主体、服务注册或菜单组件。替代提供方可以内联该精确入口，同时共享 `@deepseek-ai/dsh-client-runtime/client`。所得浏览器产物运行时不需要默认提供方的模块行；消费方负责实例销毁。`ReferenceInsert.marker` 仅控制显示（默认 `@`，也可为 `#`），与来源路由和序列化相互独立。
