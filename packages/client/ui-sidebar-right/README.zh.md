...
store 的 `closeTab` 与套件的 `canCloseTab` 共用关闭策略。保留的默认页和唯一停靠引导页不显示关闭按钮或菜单关闭项；编程式关闭也不记录任何操作。其他 tab 独自留下时，关闭会连同整列一起收起并记录一条历史，布局保持为空直到下次展开。浮动 tab 可以自由关闭，整列收起时仍然渲染。
