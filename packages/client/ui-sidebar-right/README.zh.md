...
Escape 与全屏按钮共用退出动作：宽屏恢复普通右栏，窄屏收起自动全屏面板，不关闭标签。菜单、弹窗、列表框、原生选择控件及编辑器已处理的 Escape 优先；输入法组词、带修饰键的组合键和长按重复按键不会退出全屏。
...
store 的 `closeTab` 与套件的 `canCloseTab` 共用关闭策略。保留的默认页和唯一停靠引导页不显示关闭按钮或菜单关闭项；编程式关闭也不记录任何操作。没有扩展条目时，次键按下安静的引导 chip 也不弹出菜单。其他 tab 独自留下时，关闭会连同整列一起收起并记录一条历史，布局保持为空，直到下次展开时创建当时的默认页。浮动 tab 可以自由关闭，整列收起时仍然渲染。
...
带有 `guide` 入口的页面定义可以设置 `retainAsDefault: true`。选中的页面会保留在每个展开的停靠格首位，包括直接打开文件的情况。关闭或替换此页会被拒绝；打开文件时保留此页并聚焦文件。移走或浮出此页会回填导航，不抢焦点。移除此配置即可解除保护。默认不启用，因此普通引导页和资源的行为不变。

页面定义还可以独立设置 `hideAddTab: true`，隐藏包含此类型的停靠格中的新标签页按钮，即使当前选中的是其他标签。默认不启用。此配置只改变按钮可见性：`openTab`、`openResource`、文件标签关闭、分栏和全屏控件的行为不变。注销定义后恢复当前生效定义的策略，不修改布局。
...
启用 `retainAsDefault` 的页面入口优先作为默认页。多个保留入口之间取最低的 `guide.order`，同序时按注册顺序选择。没有此配置时，恰好一个已注册引导入口会直接打开对应页面，没有或有多个入口则打开引导页。显式添加引导页仍然打开引导页。保留页面和唯一停靠引导页不可关闭；chip、上下文菜单与 `close` API 使用同一规则。
