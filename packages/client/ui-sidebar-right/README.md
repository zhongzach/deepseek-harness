...
Escape uses the same exit action as the fullscreen button: it restores the normal panel on wide viewports or collapses automatic fullscreen on narrow viewports, without closing tabs. Menus, dialogs, listboxes, native select controls and editor-consumed Escape take precedence; IME composition, modified key chords and key repeat do not exit fullscreen.
...
The store's `closeTab` and the kit's `canCloseTab` share one close policy. The retained default page and the sole docked guide have no close control or menu close item; programmatic closing records nothing. With no extension item contributed, a secondary press on the quiet guide chip opens no menu. Any other tab standing alone closes together with the column in one entry, leaving an empty layout until the next expansion seeds its current default page. Floating tabs close freely and render even while the column is collapsed.
...
An entry whose page opts into `retainAsDefault` takes precedence as the default. The lowest `guide.order` wins among multiple retained entries; registration order breaks equal orders. Without that opt-in, exactly one registered guide entry opens its page directly, while zero or multiple entries open the guide. Explicitly adding a guide still opens the guide. Retained pages and the sole docked guide cannot close; the chip, context menu, and `close` API apply the same rule.
