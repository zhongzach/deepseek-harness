...
`CommandUiContract.dismissPopups(commandName)` closes existing popups for that command when their options become stale. It aborts pending option reads and ignores their late results without creating session controllers, consuming command text, changing focus, or closing other commands' popups.

Popup options may declare `disabled` and `disabledReason` for visible informational rows. The shell exposes the reason in a tooltip and refuses pointer, keyboard, and direct-controller selection before invoking `onSelect` or consuming the command input. Disabling a row is a UI affordance; the operation owner still enforces its permission at execution.

A disabled option's optional `action` supplies an opaque id and label for a separate footer button; the registration provides `onAction` to open its help. The footer deduplicates actions from currently filtered rows. Activating help dismisses the popup before calling `onAction`, without invoking `onSelect`, consuming command text, or returning focus to the composer over the help dialog. Enabled options never contribute disabled-row actions.
