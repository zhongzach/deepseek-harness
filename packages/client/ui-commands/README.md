...
Popup rows may provide `disabled`, `disabledReason`, and a deployment-owned help action. Disabled rows remain inert for pointer and keyboard selection; deduplicated help buttons render in a separate footer, close the popup, and dispatch `onAction` without consuming the command token or submitting the denied selection. A command source's discovery policy does not change explicit command execution.
