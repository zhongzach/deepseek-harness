...
部署侧展示分组可在保留提供方与模型标识的情况下拆分目录。WriterX Hub 排在直连 DeepSeek 与其他提供方之前，Host 策略可直接省略不可用条目。不可选择的条目不能提交，但可提供独立的引导动作。授权变化时先同步清空前一账号的目录条目，再重新加载共享目录；加载失败保留结构化错误详情供产品恢复流程使用。
...
`ModelDirectoryResolver` 为每个 Host 代次持有一个共享 `ModelCatalogDirectory`，为每个 Session 持有一个 `ModelDirectory`。全局 `session.modelCatalog` 响应提供分组与可路由提供方，各 Session 的持久化 `modelSelection` 投影提供当前路由。两个 UI 入口都通过 `session.selectModel` 选择。目录与选择操作拥有独立代次校验；连接或授权更替清空旧的可选条目。`llm/adapters-updated`、`settings/document-updated` 与 `credentials/reference-updated` 刷新共享目录。会话作用域释放时删除对应目录与订阅。
