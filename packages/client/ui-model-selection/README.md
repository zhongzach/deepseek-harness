...
Deployment presentation sections split a provider's catalog without changing provider/model identities. WriterX Hub groups precede direct DeepSeek and other providers; the Host policy may omit unavailable rows entirely. Unselectable entries cannot submit and may advertise a separate assistance action. Authorization changes synchronously clear prior-account catalog rows before the shared catalog reloads; load failures preserve structured error details for product recovery.
...
Two entries over ONE per-session directory owned by `ModelDirectoryResolver` (`ctx.modelDirectories`): the `/model` popupSelect contribution (registered through `ctx.commandUi`) and the composer's named `conversation.input.model` seat both load the session's available directory through `session.models` and submit through `session.selectModel` via the same `ModelDirectory` instance, so a switch made in either entry is what the other shows next. Directory loads and selections share a generation counter so an older response never overwrites a newer one. The directory publishes the latest submitted selection as `pending` until it settles or a connection reset invalidates it; a connection reset drops every resident projection and repulls the Host-restored selection before display. Authorization replacement clears every resident directory's selectable rows before the shared catalog reloads, and an advertised but unselectable row is refused before the RPC. Directories are per-session, resolved lazily, and disposed with the session scope; addressed subagent sessions expose neither entry. Every resident directory refetches directly on forwarded `llm/adapters-updated`, `settings/document-updated`, and credential update events.
