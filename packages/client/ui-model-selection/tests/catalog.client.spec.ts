...
describe('ModelCatalogDirectory', () => {
  it('retains a rejected catalog reason for product login and membership recovery', async () => {
    const error = { code: 'session/request-denied', message: 'Sign in to continue', details: { reasonCode: 'LOGIN_REQUIRED' } }
    const subject = directory(async () => ({ ok: false, error }))
    await expect(subject.load()).rejects.toMatchObject({ code: error.code, details: error.details })
  })
