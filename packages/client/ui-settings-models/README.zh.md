...
声明步骤在 `src/onboarding-copy.ts` 中持有完整文案和版本。回环访问会通过既有 settings API 比较并写入 `ui-onboarding.welcomeNoticeVersion`；只有明确点击「继续」才会记录当前版本。组合若关闭了声明（`ui-onboarding.welcomeNotice: false`，即 `ui-settings-general` 配置的 base 层），则视同已确认，步骤直接完成而不渲染。非回环浏览器无法使用这项仅限 Host 的 namespace，因此确认仅在当前进程有效，重载后声明会再次出现。
