...
Host 配置 `welcomeNotice` 默认为 true，并落为 `ui-settings-general` 命名空间的组合 base 层。提供独立引导的产品可关闭该提示，同时保留功能自有的引导槽位与用户偏好。导航与引导投影按每个槽位 id 的获胜 entry 取值，因此产品覆盖会替换原条目而不产生重复项。
...
功能插件可通过 `settings.section.icon` 提供装饰性导航图标，以自己的分区 id 为 key。外壳传入 `size: 16`，图标不参与可访问名称。没有注册图标的 key 使用外壳的默认分区图标；图标注册不改变分区顺序、选中状态或内容。
...
宿主端在 `ui-settings-general` 条目的 Config 中把 `welcomeNoticeVersion` 声明为 volatile 字段。`ui-settings-models` 提供的欢迎步骤通过既有公开 settings 边界读写其中的 `welcomeNoticeVersion`；外壳本身仍不持有产品策略。由组合决定的唯一一项产品级事实也走同一个条目的 `base` 层：插件配置 `welcomeNotice`（默认 `true`）落为 `ui-settings-general.welcomeNotice`，于是自带引导流程的下游组合用一行配置就能关掉声明，而用户自己的设置文档仍可覆盖它。
