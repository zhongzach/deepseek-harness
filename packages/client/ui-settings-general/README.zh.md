...
宿主端在用户设置 seam 中注册 `ui-onboarding`。`ui-settings-models` 提供的欢迎步骤通过既有公开 settings 边界读写其中的 `welcomeNoticeVersion`；外壳本身仍不持有产品策略。由组合决定的唯一一项产品级事实也走同一个 namespace 的 `base` 层：插件配置 `welcomeNotice`（默认 `true`）落为 `ui-onboarding.welcomeNotice`，于是自带引导流程的下游组合用一行配置就能关掉声明，而用户自己的设置文档仍可覆盖它。
