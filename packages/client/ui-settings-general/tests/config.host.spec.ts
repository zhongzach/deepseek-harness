...
it('defaults the composition welcome-notice switch on and honors a composition override', async () => {
  const ctx = new Context()
  onTestFinished(() => ctx.fiber.dispose())
  const configuration = await liveConfig(ctx, General)
  expect(plainConfig(configuration.fiber.config)).toMatchObject({ welcomeNotice: true })
  const switched = await liveConfig(ctx, General, { welcomeNotice: false })
  expect(plainConfig(switched.fiber.config)).toMatchObject({ welcomeNotice: false })
})
