...
describe('ui-settings-general apply', () => {
  it('projects nav rows and onboarding steps from the shadowing winners: a same-id lower-priority entry replaces, not adds', async ({ mock, start }) => {
    const { c } = await client(mock, start)
    const b = { ctx: c.ctx, slots: c.ctx.slots }
    const root = b.slots.entries('sidebar.settings')[0]!
    const { hooks } = (root.inject as unknown as () => SettingsRootInjected)()
    expect(hooks.sections.getSnapshot().filter(row => row.id === 'general')).toHaveLength(1)
    ...
    expect(hooks.sections.getSnapshot().filter(row => row.id === 'general')).toEqual([{ id: 'general', order: 0, label: 'Mine' }])
    ...
    expect(hooks.onboardingSteps.getSnapshot().filter(row => row.id === 'step')).toEqual([{ id: 'step', order: 0 }])
    off()
    expect(hooks.sections.getSnapshot().find(row => row.id === 'general')?.label).toBe('通用设置')
