...
The Host `welcomeNotice` configuration defaults to true and lands in the `ui-settings-general` namespace's composition base layer. A product with its own onboarding can disable that notice while keeping feature-owned onboarding slots and user preferences. Navigation and onboarding projections use the winning entry for each slot id, so a product override replaces the original row instead of duplicating it.
