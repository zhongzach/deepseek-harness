...
The Host half registers `ui-onboarding` in the user-settings seam. The welcome step contributed by `ui-settings-models` reads and writes its `welcomeNoticeVersion` through the existing public settings boundary; the shell itself remains policy-free. The one product-wide fact the composition decides rides the same namespace as its `base` layer: the plugin config `welcomeNotice` (default `true`) lands as `ui-onboarding.welcomeNotice`, so a downstream composition that owns its own onboarding switches the notice off with one config line, and a person's settings document can still override it.
