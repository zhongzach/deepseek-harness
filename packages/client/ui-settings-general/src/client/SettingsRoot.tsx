 ...
 * panel (compact 800x640, derived from figma 501:29947) with the section nav
 * rail. The shell is a pure composition face — slot-owned text (trigger
 * label, panel title,
        ...
        <nav className={css.nav}>
          ...
          <div className={css.navList}>
            {rows.map(row => (
              ...
              >
                <span className={css.navIcon} aria-hidden="true" data-settings-nav-icon={row.id}>
                  {renderSlot('settings.section.icon', { size: 16 }, { entryKey: row.id, fallback: navIcon(row.id) })}
                </span>
