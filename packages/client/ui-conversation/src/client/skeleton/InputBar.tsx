...
export const InputBar = memo(function InputBar({
  ...
  retryFileUpload,
  toggleCommandMenu, openSource, insertReference, stop, t,
  ...
  const openLauncherSource = (source: string, trigger: '/' | '@'): void => {
    if (keyboard === undefined || locked || machineBusy) return
    const selection = keyboard.caretSpan()
    editor?.getRootElement()?.focus({ preventScroll: true })
    // A focus commit retracks the literal draft and can close a synthetic
    // source hit. Open the source only after that commit has settled.
    editor?.focus(() => { openSource?.(source, trigger, selection) })
  }

  const insertLauncherReference = (reference: ReferenceInsert): boolean => {
    if (keyboard === undefined || locked || machineBusy || insertReference === undefined) return false
    if (!insertReference(reference, keyboard.caretSpan())) return false
    editor?.getRootElement()?.focus({ preventScroll: true })
    editor?.focus()
    return true
  }
