...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    'conversation.input.launcher': { kind: 'single'; scope: 'session-maybe'; owner: ComposerLauncherOwnerProps }
...
export interface ConversationInjected {
  ...
  hooks: {
    ...
    /** Composition-owned blank-session prompt in the active locale. */
...
export interface ComposerBarInjected {
  ...
  /** Open one registered source at the editor's current selection. */
  openSource: ((source: string, trigger: '/' | '@', selection: EditSelection) => void) | undefined
  ...
  hooks: {
    ...
    /** Composition-owned ordinary-input prompt in the active locale. */
    inputPlaceholder: ObservableSnapshot<string | undefined>
  }
}

/** Product launcher callbacks bound to the active resident editor. */
export interface ComposerLauncherOwnerProps {
  /** Whether input is locked or a submission transaction is active. */
  locked: boolean
  /** Open a source without changing the draft or its current selection. */
  openSource: (source: string, trigger: '/' | '@') => void
  /** Insert one structured reference at the current selection. */
