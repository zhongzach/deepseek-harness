...
export class ReferenceChipNode extends DecoratorNode<JSX.Element> {
  ...
  /** Display fallback marker retained across editor undo and serialization. */
  __marker: ReferenceInsert['marker']
   ...
   */
  constructor(insert: Omit<ReferenceInsert, 'appearance' | 'marker'> & {
    appearance?: ReferenceInsert['appearance']
    marker?: ReferenceInsert['marker']
  }, invalid = false, key?: NodeKey) {
    super(key)
    this.__source = insert.source
    this.__ref = insert.ref
    this.__label = insert.label
    this.__appearance = insert.appearance
    this.__marker = insert.marker
  ...
  override exportJSON(): SerializedReferenceChipNode {
    return {
      ...
      ...(this.__marker === undefined ? {} : { marker: this.__marker }),
  ...
  /** Visual marker when no domain glyph is requested. */
  getMarker(): ReferenceInsert['marker'] {
