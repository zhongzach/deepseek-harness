...
export function apply(ctx: Context, config: Config = Config({})): void {
  ...
  const placeholderSource = (field: 'inputPlaceholder' | 'heroPlaceholder') => ({
    getSnapshot: (): string | undefined => {
      const value = conversationSettings.getSnapshot().value?.[field]
      return ctx.locale.getLocale().active === 'zh' ? value?.zh : value?.en
    },
  ...
  const registerComposerBar = () => slots.register({
    ...
    inject: (sessionId: SessionId | undefined): ComposerBarInjected => {
      ...
      return {
        openSource: inputTriggers === undefined ? undefined : (source, trigger, selection) => {
          shell.dismissPopup()
          ...
          inputTriggers.toggleSource(source, {
            ...
            query: '',
            quoted: false,
            position: selection.start === 0 ? 'leading' : 'inline',
