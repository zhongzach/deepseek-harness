...
Products can replace the complete Hero headline through `conversation.hero.headline` and the leading composer button through `conversation.input.launcher`. The launcher receives locked state and selection-safe source-opening and reference-insertion callbacks; Lexical owns focus, revision checks, undo, and chip geometry. Host configuration can provide `inputPlaceholder` and `heroPlaceholder` dictionaries with `zh`/`en` entries through the `ui-conversation` settings base; prerequisites, plan, and steering hints retain precedence. Reference insertions may carry a visual `marker` (`@` or `#`) and a `skill` appearance while their source codec retains submission authority.
...
The header reserves a second row only when the View selector is present. A composition with a single View uses one compact header row; registering additional Views restores their tab row without a product-specific CSS override.
