...
describe('ReferenceChipNode', () => {
  it('retains product markers and skill icons through JSON, writable clones, and occurrence projection', () => {
    const editor = makeEditor()
    editor.update(() => {
      const original = $createReferenceChipNode({ ...SKILL_REF, marker: '#', appearance: 'skill' })
      const json = original.exportJSON()
      const restored = ReferenceChipNode.importJSON(json)
      $getRoot().append($createParagraphNode().append(restored))
      restored.setInvalid(true)
      expect(restored.getMarker()).toBe('#')
      expect(restored.getAppearance()).toBe('skill')
      expect(restored.getTextContent()).toBe(SKILL_REF.clipboardText)
      expect($projectComposer(idAssigner()).occurrences[0]).toMatchObject({
        marker: '#', appearance: 'skill', invalid: true, clipboardText: SKILL_REF.clipboardText,
      })
    }, { discrete: true })
  })
