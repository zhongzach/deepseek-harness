...
async function scopedBench(register?: (inputTriggers: InputTriggerService) => void) {
  ...
  let launcher: ComposerLauncherOwnerProps | undefined
  const barProps: InputBarProps = {
    ...
    renderSlot: ((key: string, owner: object, options?: { fallback?: ReactNode }) => {
      if (key === 'conversation.input.launcher') launcher = owner as ComposerLauncherOwnerProps
  ...
  return { runtime, inputTriggers, controller, shell, wiring, view, textarea, type, sink, serialize, release,
    launcher: () => {
      if (launcher === undefined) throw new Error('launcher slot not rendered')
      return launcher
    },
  }
}
...
describe('product launcher with the resident editor', () => {
  it('keeps a reference menu open after returning focus from a product button', async () => {
    const b = await scopedBench((sources) => {
    ...
    b.type('前 后')
    act(() => { b.shell.editor.update(() => { $selectDetectSpan({ start: 2, end: 2 }) }, { discrete: true }) })
    const button = document.createElement('button')
    document.body.append(button)
    button.focus()
    await act(async () => { b.launcher().openSource('skills', '@') })
    await vi.waitFor(() => {
      expect(b.controller.launcher.getSnapshot()).toBe('skills')
      expect(b.controller.menu.getSnapshot().groups[0]?.items.map(item => item.name)).toEqual(['review'])
    })
    expect(document.activeElement).toBe(b.textarea)
    act(() => { b.controller.pick('skills', 0) })
    expect(b.shell.snapshot.draft).toBe('前 /review 后')
    button.remove()
  })
})
