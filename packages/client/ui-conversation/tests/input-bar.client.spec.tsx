...
import type {
  ComposerAttachment, ComposerAttachmentsOwnerProps, ComposerLauncherOwnerProps, DraftFileUploads,
...
function launcherOwner(slotCalls: readonly { key: string; owner: unknown }[]): ComposerLauncherOwnerProps {
  const owner = slotCalls.findLast(call => call.key === 'conversation.input.launcher')?.owner
  if (owner === undefined) throw new Error('launcher slot was not rendered')
  return owner as ComposerLauncherOwnerProps
}
...
describe('Enter semantics', () => {
  it('uses configured ordinary copy without masking prerequisite, plan, or steering guidance', () => {
    ...
    expect(bench({ inputPlaceholder }).placeholder).toBe(inputPlaceholder)
    cleanup()
    expect(bench({ inputPlaceholder, placeholder: '先选择工作区' }).placeholder).toBe('先选择工作区')
    cleanup()
    expect(bench({ inputPlaceholder, disabled: true }).placeholder).toBe(zh['placeholder.unavailable'])
    cleanup()
    expect(bench({ inputPlaceholder, plan: { active: true, pending: false } }).placeholder).toBe(zh['placeholder.plan'])
    cleanup()
    expect(bench({ inputPlaceholder, running: true, queue: [row('queued')] }).placeholder).toBe(zh['placeholder.steerQueue'])
    cleanup()
    expect(bench({
      inputPlaceholder,
      subagent: {
    ...
    }).placeholder).toBe(zh['placeholder.parentOffline'])
  })
  ...
  it('locks launcher opens and inserts while the input machine is submitting', () => {
    ...
    act(() => {
      ...
      result.shell.beginCommand(
        { name: 'goal', token: '/goal ', submit: () => new Promise<never>(() => {}) },
...
describe('command launcher chrome and control seats', () => {
  ...
  it('gives a launcher current-selection-safe open and reference insertion verbs', async () => {
    ...
    act(() => { result.shell.editor.update(() => { $selectDetectSpan({ start: 7, end: 10 }) }, { discrete: true }) })
    const owner = launcherOwner(result.slotCalls)
    const launcherButton = document.createElement('button')
    document.body.append(launcherButton)
    launcherButton.focus()

    await act(async () => { owner.openSource('files', '@') })
    expect(openSource).toHaveBeenCalledExactlyOnceWith('files', '@', { start: 7, end: 10 })
    expect(document.activeElement).toBe(result.textarea)
    expect(result.shell.caretSpan()).toEqual({ start: 7, end: 10 })
    launcherButton.remove()
    let inserted = false
    await act(async () => { inserted = owner.insertReference({
      source: 'files',
      ref: 'src/a.ts',
      label: 'a.ts',
      marker: '#',
    ...
    expect(result.shell.snapshot.draft).toBe('before #src/a.ts after')
    expect(document.activeElement).toBe(result.textarea)
    expect(result.shell.caretSpan()).toEqual({ start: 8, end: 8 })
  })
