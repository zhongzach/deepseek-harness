...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared `sidebar.workspaces`, `sidebar.settings`, `sidebar.footer.action` and `sidebar.brand` child slots, and injected `startSession` plus sidebar-toggle callbacks. There is no plugin store.
...
The brand mark is the `sidebar.brand` hole: the shell renders it inside the expanded wordmark button (which stays the New Session shortcut) and again inside the rail's expand toggle, sharing only the column state (`wide`) so a registrant can pick its wordmark or its glyph. A downstream composition registers its own mark there; while the hole is unregistered the shell falls back to its own DeepSeek Harness wordmark and whale, so the shipped GUI is unchanged.
