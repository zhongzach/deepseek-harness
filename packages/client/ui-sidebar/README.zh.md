...
`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的 `sidebar.workspaces`、`sidebar.settings`、`sidebar.footer.action` 与 `sidebar.brand` 子 slot，以及注入的 `startSession` 和侧边栏切换回调。这里没有插件 store。
...
品牌标识是 `sidebar.brand` 这个 hole：外壳在展开态的字标按钮里（它仍然是 New Session 快捷入口）渲染它，在收起态的展开切换按钮里再渲染一次，只共享栏状态（`wide`），让注册方自行选择渲染字标还是图标。下游组合可以在这里注册自己的品牌标识；hole 未注册时，外壳回退到自己的 DeepSeek Harness 字标与鲸鱼图标，出厂 GUI 因此不变。
