...
function fakePanels(): PanelActions {
  return {
    ...
    setSidebar: vi.fn(),
    toggleSidebar: vi.fn(),
    collapseSidebar: vi.fn(), expandSidebar: vi.fn(),
    setShelf: vi.fn(), openShelf: vi.fn(), closeShelf: vi.fn(), toggleShelf: vi.fn(), resizeShelf: vi.fn(),
...
describe('LayoutController product actions', () => {
  it('forwards shelf and explicit sidebar actions to the same root instance', () => {
    ...
    service.closeShelf()
    service.toggleShelf()
    service.resizeShelf(720)
    expect(panels.collapseSidebar).toHaveBeenCalledOnce()
    expect(panels.expandSidebar).toHaveBeenCalledOnce()
    expect(panels.openShelf).toHaveBeenCalledOnce()
    expect(panels.closeShelf).toHaveBeenCalledOnce()
    expect(panels.toggleShelf).toHaveBeenCalledOnce()
    expect(panels.resizeShelf).toHaveBeenCalledWith(720)
    expect(panels.openRightbar).not.toHaveBeenCalled()
    expect(panels.closeRightbar).not.toHaveBeenCalled()
  })
})
