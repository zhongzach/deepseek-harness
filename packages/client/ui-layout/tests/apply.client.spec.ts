...
describe('ui-layout client apply', () => {
  ...
  it('provides ctx.layout and registers AppFrame into root with the four child declarations', async () => {
    ...
    expect(slots.spec('shelf')).toEqual({ kind: 'single', scope: 'session-maybe' })
    expect(slots.spec('details')).toEqual({ kind: 'single', scope: 'session' })
    expect(slots.spec('titlebar')).toEqual({ kind: 'single', scope: 'root' })
  })
  ...
  it('injects no business face and attaches the layout actions', async () => {
    ...
    await fiber.await()
    const actions = {
      setSidebar: vi.fn(), setShelf: vi.fn(), setDetails: vi.fn(), toggleSidebar: vi.fn(),
      setNarrow: vi.fn(), openShelf: vi.fn(), closeShelf: vi.fn(), openDetails: vi.fn(), closeDetails: vi.fn(),
    }
    ...
    expect(actions.openShelf).toHaveBeenCalledOnce()
  })
