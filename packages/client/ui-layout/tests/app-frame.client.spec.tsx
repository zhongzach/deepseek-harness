...
function mountFrame() {
  ...
  const renderSlot = ((key: string, owner: object) => {
    ...
    if (key === 'shelf') return <div data-testid="shelf-content" />
...
describe('AppFrame', () => {
  ...
  it('shelf column stays mounted at zero width and opens at the contract default', () => {
    const { frame, instance, getByTestId } = mountFrame()
    expect(tracks(frame)).toEqual([280, 0, 0])
    expect(getByTestId('shelf-content')).toBeTruthy()
    expect(frame.hasAttribute('data-shelf-collapsed')).toBe(true)
    act(() => { instance.actions.openShelf() })
    expect(tracks(frame)).toEqual([280, SHELF_DEFAULT, 0])
    expect(frame.hasAttribute('data-shelf-collapsed')).toBe(false)
  ...
  it('shelf and details can be open together, each with its own handle', () => {
    ...
    act(() => { instance.actions.openShelf() })
    act(() => { instance.actions.openDetails() })
    expect(tracks(frame)).toEqual([280, SHELF_DEFAULT, 360])
