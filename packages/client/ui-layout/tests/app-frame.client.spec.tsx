...
function mountFrame(windowWidth = frameWidth) {
  ...
  const frame = utils.container.querySelector('[class*="frame"]') as HTMLElement
  return {
...
/* The template delegates the squeeze to the grid (jsdom does no layout, so
   ...
   The centre's protected minimum must accompany an open right track; the
   product shelf stays a JS-solved fixed track between them. */
function tracks(frame: HTMLElement): number[] {
  const match = /^([\d.]+)px minmax\((0|400)px, 1fr\) ([\d.]+)px minmax\(0px, ([\d.]+)px\)$/.exec(frame.style.gridTemplateColumns)
...
function handleFor(frame: HTMLElement, side: 'sidebar' | 'shelf' | 'rightbar'): HTMLElement {
...
describe('product shell preservation', () => {
  it('uses the boot presentation title and retains titlebar and shelf outlets', () => {
    vi.stubGlobal('__DSH_BOOT_PRESENTATION__', { documentTitle: 'WriterX Hermes' })
    const { getByTestId } = mountFrame()
    expect(document.title).toBe('WriterX Hermes')
    expect(getByTestId('titlebar-content')).toBeTruthy()
    expect(getByTestId('shelf-content')).toBeTruthy()
  })

  it('keeps the shelf mounted across close and drags from its resolved width', () => {
    const { frame, instance, getByTestId } = mountFrame()
    const shelf = getByTestId('shelf-content')
    expect(frame.dataset.shelfCollapsed).toBe('true')
    act(() => { instance.actions.openShelf() })
    expect(frame.style.gridTemplateColumns).toBe('280px minmax(0px, 1fr) 300px minmax(0px, 0px)')
    const handle = handleFor(frame, 'shelf')
    expect(handle.style.left).toBe('1620px')
    drag(handle, 1620, 1420)
    expect(instance.getSnapshot().layoutInfo.shelf).toBe(500)
    act(() => { instance.actions.closeShelf() })
    expect(frame.dataset.shelfCollapsed).toBe('true')
    expect(getByTestId('shelf-content')).toBe(shelf)
    expect(frame.querySelector('[data-side="shelf"]')).toBeNull()
  })
})
