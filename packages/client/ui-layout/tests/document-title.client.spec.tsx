...
describe('product title presentation', () => {
  it('uses the WriterX title while following the official Session and panel sources', () => {
    const { sessionId, sessions, props } = titleSources()
    sessions.update((state) => { state.byId[sessionId]!.title = '第一章' })
    render(<DocumentTitle {...props} productTitle="WriterX Hermes" />)
    expect(document.title).toBe('第一章 — WriterX Hermes')
    act(() => { sessions.update((state) => { state.byId[sessionId] = { ...state.byId[sessionId]!, retainedBy: {} } }) })
