...
describe('product shelf and explicit sidebar actions', () => {
  it('opens, resizes and closes the independent shelf without changing rightbar state', () => {
    const store = createLayoutStore().create()
    const { actions } = store
    const getSnapshot = () => store.getSnapshot()
    actions.openShelf()
    expect(getSnapshot().layoutInfo.shelf).toBe(300)
    actions.resizeShelf(720)
    actions.openShelf()
    expect(getSnapshot().layoutInfo.shelf).toBe(720)
    actions.setShelf(9999)
    expect(getSnapshot().layoutInfo.shelf).toBe(1040)
    actions.resizeShelf(1)
    expect(getSnapshot().layoutInfo.shelf).toBe(300)
    actions.closeShelf()
    expect(getSnapshot().layoutInfo.shelf).toBe(0)
    actions.toggleShelf()
    expect(getSnapshot().layoutInfo.shelf).toBe(300)
    actions.toggleShelf()
    expect(getSnapshot().layoutInfo.shelf).toBe(0)
    expect(getSnapshot().layoutInfo.rightbarShown).toBe(false)
  })

  it('explicit sidebar actions are idempotent in wide and narrow modes', () => {
    const store = createLayoutStore().create()
    const { actions } = store
    const getSnapshot = () => store.getSnapshot()
    actions.collapseSidebar()
    actions.collapseSidebar()
    expect(getSnapshot().layoutInfo.sidebar).toBe(0)
    actions.expandSidebar()
    actions.expandSidebar()
    expect(getSnapshot().layoutInfo.sidebar).toBe(280)
    actions.setSidebar(400)
    actions.setViewportWidth(800)
    actions.expandSidebar()
    actions.expandSidebar()
    expect(getSnapshot().layoutInfo).toMatchObject({ sidebar: 400, narrowExpanded: true })
    actions.collapseSidebar()
    actions.collapseSidebar()
    expect(getSnapshot().layoutInfo).toMatchObject({ sidebar: 400, narrowExpanded: false })
  })
})
