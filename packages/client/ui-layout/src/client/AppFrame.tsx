 ...
 * product shelf | rightbar), the drag handles (pointer capture + rAF throttle), the column
...
export type AppFrameProps =
  & PropsRuntime<'root'>
  & PropsRenderSlots<'sidebar' | 'main' | 'shelf' | 'rightbar' | 'shell.overlay' | 'titlebar' | 'shell.leading'>
...
/** Product shelf remains mounted while its track is closed. */
function ShelfColumn(props: { children?: ReactNode }) {
  return <div className={css.shelfCol}>{props.children}</div>
}
 ...
 */
function DragHandle(props: { side: 'sidebar' | 'shelf' | 'rightbar'; left: number; onStart: () => void; onDrag: (dx: number) => void; onEnd: () => void }) {
...
export function AppFrame({
  ...
  const normal = computeColumns(
    viewport, !layoutInfo.rightbarShown && narrow ? 0 : sidebarPreference, rightbarPreference, layoutInfo.shelf, collapsedWidth,
  )
  const cols = computeColumns(
    viewport, sidebarPreference, layoutInfo.rightbarTrack ? rightbarPreference : 0, layoutInfo.shelf, collapsedWidth,
  )
  ...
  const onShelfStart = useCallback(() => { shelfBase.current = colsRef.current.shelf; setDragging(true) }, [])
    ...
    <div className={css.shell}>
      ...
      <div
        ...
        style={{
          ...
          gridTemplateColumns:
            `${cols.sidebar}px minmax(${cols.rightbar === 0 ? 0 : CENTER_MIN}px, 1fr) ${cols.shelf}px minmax(0px, ${rightbarMax}px)`,
        }}
        data-sidebar-collapsed={sidebarCollapsed || undefined}
        data-shelf-collapsed={cols.shelf === 0 || undefined}
      ...
      >
        ...
        {cols.shelf > 0 && (
          <DragHandle side="shelf" left={viewport - cols.shelf - cols.rightbar} onStart={onShelfStart} onDrag={onShelfDrag} onEnd={onDragEnd} />
        )}
