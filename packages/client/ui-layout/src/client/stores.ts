...
import {
  ...
  SHELF_DEFAULT, SHELF_MIN, SHELF_MAX,
} from './columns.ts'
...
type LayoutInfo = {
  sidebar: number
  /** Independent product shelf width; zero keeps its resident subtree closed. */
  shelf: number
...
export function createLayoutStore(): EngineStoreHandle<LayoutState, LayoutActions>  {
  const handle = defineStore({
    ...
    actions: {
      ...
      expandSidebar: (d) => {
        d.layoutInfo.rightbarInstant = false
        if (d.layoutInfo.viewportWidth < SIDEBAR_AUTO_COLLAPSE) d.layoutInfo.narrowExpanded = true
        else if (d.layoutInfo.sidebar === 0) d.layoutInfo.sidebar = SIDEBAR_DEFAULT
      },
      setShelf: (d, px: number) => { d.layoutInfo.shelf = clampWidth(px, SHELF_MIN, SHELF_MAX) },
      openShelf: (d) => { if (d.layoutInfo.shelf === 0) d.layoutInfo.shelf = SHELF_DEFAULT },
      closeShelf: (d) => { d.layoutInfo.shelf = 0 },
      toggleShelf: (d) => { d.layoutInfo.shelf = d.layoutInfo.shelf === 0 ? SHELF_DEFAULT : 0 },
      resizeShelf: (d, px: number) => { d.layoutInfo.shelf = clampWidth(px, SHELF_MIN, SHELF_MAX) },
