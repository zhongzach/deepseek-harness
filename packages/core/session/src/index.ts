...
import type {
  ...
  SessionEventMap, SessionEventType, SessionHeader, SessionId, SessionSeedEventState, SurfaceEventType, SurfaceIntent,
} from './types.ts'
...
export { isKnownSessionEventType, knownSessionEventTypes, registerSessionEventType } from './deployment-event-types.ts'
...
export class Session {
   ...
   *   Assistant messages embed their exact provider stream and cannot cite
   *   top-level source events.
  ...
  append<T extends SessionEventType>(
    ...
    ...opts: T extends SurfaceEventType ? [opts: SurfaceIntent<T> & AppendIntent] : [opts?: AppendIntent]
  ): SessionEvent<T> {
...
export class SessionStore extends Service {
  ...
  async flush(session: Session): Promise<boolean> {
    ...
    // Async invocation preserves each listener's exact thrown/rejected value.
