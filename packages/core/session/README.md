...
Informational deployment records may pass `{ ignorable: true }` at append time. `registerSessionEventType` also admits same-format records written before that marker existed; overlapping registrations remain active until their last owner disposes. Historical format migration requires [an explicit payload validator](../../../.agents/notes/implemented/architecture/2026-09-05-deployment-event-format-migration.md).
