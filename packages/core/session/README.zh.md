...
信息型部署记录可以在追加时传入 `{ ignorable: true }`。`registerSessionEventType` 还可接受在该标记出现之前写入的同格式记录；重叠注册会保持有效，直到最后一个所有者卸载。历史格式迁移要求[显式载荷校验器](../../../.agents/notes/implemented/architecture/2026-09-05-deployment-event-format-migration.zh.md)。
