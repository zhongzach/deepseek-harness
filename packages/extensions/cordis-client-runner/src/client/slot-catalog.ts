...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  },
  {
    ...
    kind: 'chain',
    scope: 'session',
    summary: 'Conditional presentation takeover for one durable context node.',
    doc: 'Conditional presentation takeover for one durable context node. The\ndefault context renderer remains the fallback, so a product can claim a\ntechnical producer without copying or replacing every context form.',
    registerOptions: [
      {
        name: 'select',
        requirement: 'required',
  ...
  },
  {
    ...
    kind: 'single',
    scope: 'session-maybe',
    summary: 'Single launcher at the leading edge of the composer tool row.',
    doc: 'Single launcher at the leading edge of the composer tool row. The bar\nretains its command-plus fallback when no entry is registered. An\noccupant receives current-selection-safe verbs and never constructs a\ntoken span itself.',
    registerOptions: [],
    ownerProps: [
