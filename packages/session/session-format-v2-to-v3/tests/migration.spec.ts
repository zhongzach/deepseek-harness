...
import { createSessionFormatCatalog, isSessionFormatJsonObject, registerOpaqueSessionEvent, SessionFormatEventCollector } from '@deepseek-ai/dsh-session-format'
...
describe('streaming V2 system prompt migration', () => {
  it.each([0, 1, 2])('preserves validated deployment metadata across V%s to V3 without treating counters as references', (version) => {
    const type = 'deployment/migration-audit'
    const dispose = registerOpaqueSessionEvent(type, {
      versions: [0, 1, 2],
      validateData(data) {
        if (data !== null && (!isSessionFormatJsonObject(data) || data['charged'] !== 2)) {
          throw new Error('invalid deployment receipt')
        }
      },
    })
    try {
      const physical = version === 2
        ? { type: 'session', ...header }
        : { type: 'session', version, id: header.id, createdAt: 1, delegationDepth: 0 }
      const restore = catalog.createRestore(physical, { recovery: 'strict', validation: 'current' })
      const source = dense([
        ...opening(),
        event(type, { charged: 2, requestOrdinal: 999 }),
        event(type, null),
      ...
      expect(output.header.version).toBe(3)
      expect(output.events.filter(row => row.type === type)).toEqual([
        { ...source[2], seq: 3 },
        { ...source[3], seq: 4 },
      ])
    } finally {
    ...
    expect(() => migrate([event(type, { charged: 2 })])).toThrow(/unclassified event/)
  })

  it('refuses malformed or reference-bearing deployment events at the migration boundary', () => {
    const type = 'deployment/migration-refusal'
    const dispose = registerOpaqueSessionEvent(type, {
      versions: [2],
      validateData(data) {
        if (data !== 'valid') throw new Error('invalid deployment receipt')
      },
    })
    try {
      expect(() => migrate([event(type, 'invalid')])).toThrow(/invalid deployment receipt/)
      expect(() => migrate([{ ...event(type, 'valid'), sourceEventSeqs: [0] }])).toThrow(/log-only without sequence references/)
      expect(() => migrate([event(type, 'valid', 'append')])).toThrow(/log-only without sequence references/)
    } finally {
      dispose()
    }
  })
