...
import { SessionFormatError, SessionFormatUnsupportedMigrationError, isRegisteredOpaqueSessionEvent, isSessionFormatJsonObject, sessionFormatCount, sessionFormatSafeInteger } from '@deepseek-ai/dsh-session-format'
...
export function assertEvent(event: SessionFormatEvent, version: 2 | 3): void {
  ...
  const opaque = disposition === undefined && !feedback && isRegisteredOpaqueSessionEvent(event, 2)
  if (disposition === undefined && !feedback && !opaque) {
