...
import { SessionFormatError, SessionFormatUnsupportedMigrationError, defineSessionFormatMigration, isRegisteredOpaqueSessionEvent, sessionFormatCount } from '@deepseek-ai/dsh-session-format'
...
import { assertReleasedV2Header, RELEASED_V2_EVENT_DISPOSITIONS } from '@deepseek-ai/dsh-session-format-v1-to-v2'
...
class ReleasedV2ToV3Stage implements SessionFormatMigrationStage {
  ...
  transformEvent(event: SessionFormatEvent, context: SessionFormatMigrationContext): void {
    ...
    assertEvent(event, 2)
    if (RELEASED_V2_EVENT_DISPOSITIONS[event.type] === undefined && isRegisteredOpaqueSessionEvent(event, 2)) {
      const seq = this.targetSeq++
      this.mapping.push(seq)
      context.emitEvent({ ...event, seq })
      return
    }
