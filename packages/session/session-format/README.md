...
A deployment plugin may register validated informational payloads through `registerOpaqueSessionEvent` before opening stored sessions. The registration names exact format versions and guarantees no embedded Session sequence references; surface metadata is rejected, while accepted payloads remain unchanged. Its disposer follows the plugin lifetime. [Deployment event migration](../../../.agents/notes/implemented/architecture/2026-09-05-deployment-event-format-migration.md) owns the preservation and refusal rules.
