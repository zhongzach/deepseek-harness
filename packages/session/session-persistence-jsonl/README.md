...
Worker verification receives the caller's immutable snapshot of known Session event names, captured before queueing. It registers those names only for that verification and releases them afterward; it never discovers names from the staged file. Historical deployment payloads still require the migration chain's explicit validators, and current generation structure, digest, and source revision checks remain mandatory.
