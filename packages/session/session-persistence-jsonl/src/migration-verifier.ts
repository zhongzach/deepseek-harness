...
/**
 * Verify one current generation in a fresh Worker Thread using the event types
 * registered when verification is requested, including while waiting for a worker.
...
export function verifyCurrentGenerationInWorker(
  ...
  const request: VerificationRequest = {
    ...
    knownEventTypes: Object.freeze([...knownSessionEventTypes()]),
  }
  return verificationScheduler.run(() => runVerificationWorker(request, signal), signal)
}
