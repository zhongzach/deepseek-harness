...
import { knownSessionEventTypes, registerSessionEventType } from '@deepseek-ai/dsh-session'
...
describe('migration verifier Worker lifecycle', () => {
  it('resolves only after terminating a successful Worker', async () => {
    ...
    expect(instance.options.workerData).toEqual({
      ...
      expectedPrefix,
      knownEventTypes: [...knownSessionEventTypes()],
    })
  ...
  it('captures registered event types before queuing and excludes later registrations', async () => {
    const first = verifyCurrentGenerationInWorker('/first', 'none', 'session', 0)
    const second = verifyCurrentGenerationInWorker('/second', 'none', 'session', 0)
    const dispose = registerSessionEventType('deployment/queued-verification')
    const queued = verifyCurrentGenerationInWorker('/queued', 'none', 'session', 0)
    dispose()
    const disposeLater = registerSessionEventType('deployment/later-verification')
    try {
      worker(0).emit('message', { ok: true, result })
      await first
      await vi.waitFor(() => { expect(state.workers).toHaveLength(3) })
      const request = worker(2).options.workerData as { knownEventTypes: readonly string[] }
      expect(request.knownEventTypes).toContain('deployment/queued-verification')
      expect(request.knownEventTypes).not.toContain('deployment/later-verification')
      expect(Object.isFrozen(request.knownEventTypes)).toBe(true)
    } finally {
      disposeLater()
      worker(1).emit('message', { ok: true, result })
      worker(2).emit('message', { ok: true, result })
    ...
    expect(knownSessionEventTypes().has('deployment/queued-verification')).toBe(false)
    expect(knownSessionEventTypes().has('deployment/later-verification')).toBe(false)
  })
