...
import { registerSessionEventType, SESSION_FORMAT_VERSION, SessionId } from '@deepseek-ai/dsh-session'
...
describe('deployment-owned current events', () => {
  it('retains flagless events only while their owner is registered', () => {
    const makeEvent = () => ({ type: 'deployment/charge', seq: 0, time: 1, data: { charged: 1 } }) as unknown as SessionEvent
    expect(() => validateStoredEvents(meta('deployment'), [makeEvent()])).toThrow(SessionFormatUnsupportedError)
    const unregister = registerSessionEventType('deployment/charge')
    try {
      expect(validateStoredEvents(meta('deployment'), [makeEvent()])[0]).toEqual(makeEvent())
    } finally {
      unregister()
    }
    expect(() => validateStoredEvents(meta('deployment'), [makeEvent()])).toThrow(SessionFormatUnsupportedError)
