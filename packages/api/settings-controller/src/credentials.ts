...
export class CredentialsController extends TypertRemoteService {
  ...
  async set(ref: string, value: string): Promise<void> {
    ...
    const credentials = this.provider()
    await authorizeApiOperation(this.ctx, { method: 'credentials.set', payload: { ref: request.ref } })
  ...
  async unset(ref: string): Promise<void> {
    ...
    const branded = credentialRef(request.ref)
    const credentials = this.provider()
    await authorizeApiOperation(this.ctx, { method: 'credentials.unset', payload: { ref: request.ref } })
