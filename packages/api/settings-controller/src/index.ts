...
import { authorizeApiOperation } from '@deepseek-ai/dsh-api-operation-authorization'
import { redactSettingsOperation } from './authorization.ts'
import {
  openNativeTextFile,
...
export class SettingsController extends TypertRemoteService {
  ...
  private async write(
    ...
    const previous = settings.describe({ redactSecrets: true }).find(candidate => candidate.ns === namespace)
    const revision = expectedRevision === undefined ? {} : { expectedRevision }
    const operation = mode === 'update'
      ? { method: 'settings.update' as const, payload: { ns, patch: input, ...revision } }
      : mode === 'replace'
        ? { method: 'settings.replace' as const, payload: { ns, section: input, ...revision } }
        : { method: 'settings.mutate' as const, payload: { ns, ops: input as SettingsPathOp[], ...revision } }
    await authorizeApiOperation(this.ctx, redactSettingsOperation(previous, operation))
    try {
