...
如果空白 Session 记录的 preset 已不存在，可以采用部署的默认 preset。controller 在异步 preset setup 前后重新检查是否尚未开始对话，并在恢复前记录 `agent-preset/selected`。已有对话历史的 Session 仍要求原 preset；preset 不可用时，绝不静默切换已有对话。

可选的 [`operation-authorization`](../operation-authorization/README.zh.md) 策略接口在模型选择变更和默认值持久化之前执行。目录策略接收独立拷贝的模型目录，可以提供展示信息、可用性和产品操作，而不改变底层 provider 路由。
