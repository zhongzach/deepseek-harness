...
export class ApiSessionAgentController {
  ...
  /** Resolve a stored composition, recording a default only for a blank missing-preset Session. */
  private async composeStoredAgent(observation: SessionObservation): Promise<{ readonly setup: AgentSetup }> {
    try {
      return await this.composeAgent(this.presetForObservation(observation))
    } catch (error: unknown) {
      if (remoteErrorOf(error)?.code !== 'agent-preset/not-found'
      ...
      return {
        setup: async (agentCtx, agent) => {
          ...
          const assertBlank = (): void => {
            // oxlint-disable-next-line typescript/no-deprecated -- Existing blank-preset resume fence; deferred projection migration.
          ...
          return {
            commit: () => {
              ...
              setupCommit?.commit()
              assertBlank()
              if (replacement.agentPreset !== undefined) {
                session.append('agent-preset/selected', { agentPreset: replacement.agentPreset })
              }
            },
          }
        },
  ...
  private async resumeObserved(
    ...
    const composition = await this.composeStoredAgent(observation)
