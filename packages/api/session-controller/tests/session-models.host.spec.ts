...
import { ApiAuthorizationError } from '@deepseek-ai/dsh-api-operation-authorization'
...
describe('Web session model selection', () => {
  it('refuses a selection before changing the durable session or its default', async () => {
    ...
    expect(await remote.selectModel({ sessionId, provider: 'deepseek-official', model: 'deepseek-reasoner' }))
      .toMatchObject({ ok: false, error: {
        code: 'api/operation-denied', message: 'Membership required.',
    ...
    reject()
    expect((await remote.selectModel({ sessionId, provider: 'deepseek-official', model: 'deepseek-reasoner' })).ok).toBe(true)
  ...
  it('does not convert an authorization outage into model unavailability or apply a partial selection', async () => {
    ...
    const response = await remote.selectModel({ sessionId, provider: 'deepseek-official', model: 'deepseek-reasoner' })
    expect(response).toMatchObject({ ok: false, error: {
      code: 'api/operation-denied', details: { reasonCode: 'AUTHORIZATION_UNAVAILABLE', retryable: false },
    } })
    expect(JSON.stringify(response)).not.toContain('private authorization failure')
    expect(saved).not.toHaveBeenCalled()
    expect(currentSelection(ctx, sessionId).model).toBe('deepseek-chat')
    await ctx.fiber.dispose()
  })
  ...
  it('publishes deployment availability for both model catalogs without filtering locked rows', async () => {
    ...
    for (const catalog of [expectValue(await remote.modelCatalog()), await buildModelCatalog(ctx)]) {
