...
A blank Session whose recorded preset no longer exists may adopt the deployment default. The controller rechecks that no conversation has started before and after asynchronous preset setup, then records `agent-preset/selected` before resuming. Sessions with conversation history retain their original preset requirement; an unavailable preset never silently switches an existing conversation.

The optional [`operation-authorization`](../operation-authorization/README.md) policy seam runs before model selection changes or default persistence. Its catalog policy receives a detached model catalog and may supply presentation, availability, and product actions without changing the underlying provider routes.
...
`skills/list` preserves each winning skill's optional `displayName` for Client presentation. Clients invoke the skill by `name`, not by this title; listing titles does not activate an Agent or issue a model request.
